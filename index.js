...
client.on("interactionCreate", async interaction => {
    ...
    // Edit the original ephemeral reply with confirmation buttons
    ...
    collector.on("collect", async btnInteraction => {
        if (btnInteraction.customId === "accept") {
            ...
            await btnInteraction.update({ content: `✅ ${command} confirmed for user ${userId}`, embeds: [], components: [] });
        } else if (btnInteraction.customId === "cancel") {
            await btnInteraction.update({ content: `❌ ${command} canceled`, embeds: [], components: [] });
        }
        collector.stop();
    });
